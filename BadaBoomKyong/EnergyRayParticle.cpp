...
void EnergyRayParticle::CreateParticles(int _emitNum, glm::vec3 _start, glm::vec3 _end)
{
    ...
    float lengthX=std::fabs(_end.x-_start.x);        
    ...
    for (int i = 0; i < beam_center; i++)
    {
        ...
        Particle p;
        p.m_vPosition = _start + (dir *length)/2.f;
        p.m_fRotation = angle;
        ...
        p.m_vSizeStart = { length+10000.f,30.f };
        p.m_vSize = { length + 10000.f,30.f };
        p.m_vSizeEnd = { length,0.f };
        m_pParticleSystem->Emit(p, behavior, 1);
    }

    Particle p_side;
    p_side.m_vPosition = { _start + (dir * length) / 2.f };
    p_side.m_fRotation = angle;
